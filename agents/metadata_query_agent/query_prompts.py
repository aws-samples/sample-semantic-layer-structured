...
# schema patterns (label-vs-code column, bridge/junction table, self-join,
# derivation/substitution) in neutral terms and must NOT hardcode any specific
# layer's table/column names. Layer-specific join/derivation knowledge belongs in
# the table/column DESCRIPTIONS carried in the slice (the metadata agent writes
# these), which the judge reads — never here, so this judge generalizes to every
# semantic layer.
JUDGE_PROMPT = (
    ...
    "labels for the same column. Read a column's or table's DESCRIPTION as "
    "authoritative guidance — when it names the join key, the lookup, or the "
    "derivation to use, honor it rather than demanding a separate concept.\n"
    ...
    "its coded sibling (its values are themselves descriptive words, so it needs "
    "no further lookup). Likewise a column whose values are inherently readable (a "
    "name, a type word) needs no further lookup. Do NOT demand a separate lookup "
    "table when such a label/value column is present — that is over-rejection. "
    "When path (a) applies (a label or inherently-readable column is present), "
    "confirm sufficient=true on that basis ALONE and do NOT put any lookup/"
    "reference table in missing[] or treat its absence as a gap — the readable "
    "value is already available in a direct column, so the SQL must read it "
    "directly and must NOT introduce a JOIN to a code/lookup table (an INNER JOIN "
    "to a lookup whose codes don't line up silently drops ALL rows → a wrong "
    "0-row answer).\n"
    "  (b) Only when NO readable label column exists for the coded value AND a "
    "separate lookup/reference table (a code→description table joined on the code) "
    "would be required, and that lookup table/column is absent from the "
    ...
    "require a particular schema/database qualifier from the question text. Any "
    "layer/database name the USER typed (e.g. words like 'curated', 'normalized', "
    "'raw', or 'the … layer') is prose describing the dataset, NOT part of a "
    "required table id; do not treat it as a qualifier the slice must match. If "
    ...
    "name, and do NOT fabricate a schema-qualified id (a `<schema>.<table>` string "
    "you constructed) that does not appear in the slice.\n"
    ...
    "the bare name IS present, do NOT add it — a schema prefix you constructed is "
    "not a required qualifier. Only keep an entry "
    ...
    "named business entity and the slice contains both a PRIMARY entity table AND "
    "a derived/associative table for that entity (a table whose name extends the "
    "entity's name with a role/link suffix), set sufficient=false and name the "
    "primary entity table in missing[] unless the primary table is explicitly "
    "present in the slice. Do not treat a derived or associative table as a "
    "substitute for counting the primary entity.\n"
    "Relationship connectivity (also OVERRIDES the join-path bias): when the "
    "question RELATES two entities (e.g. 'measure of X items grouped BY Y', 'X and "
    "their related Z'), verify the slice actually contains a join PATH connecting "
    "those two tables — either a direct foreign key, or a bridge / junction / "
    "intermediate table that joins to BOTH (two entity tables may not join "
    "directly and connect only through a third table that carries both their "
    "keys). A multi-hop path counts: if entity A joins to a bridge table and that "
    "bridge (directly or via another present table) carries an FK to entity B, the "
    "two are connected and the relationship IS satisfied — set sufficient=true on "
    "that basis. A bridge that joins to BOTH is fully equivalent to a direct "
    "foreign key: when such a bridge is present (a table holding both entities' "
    "keys), do NOT additionally demand a direct FK on an entity table, and do NOT "
    "invent a missing direct-FK column or require a generic 'relation' table — "
    "naming an absent direct-FK column or a redundant linking table when a "
    "connecting bridge already exists in the slice is over-rejection. Only set "
    "sufficient=false for connectivity when the two tables are present but NOTHING "
    "in the slice connects them — then name the bridge / junction table you'd add "
    "in missing[] (the builder will pull it in). If NO table in this dataset can "
    "connect them — the relationship the question needs simply is not modelled — "
    "set sufficient=false and name the missing linking table; do NOT pass a slice "
    "the SQL generator could only satisfy by inventing a join column that does not "
    "exist.\n"
    ...
    "(do NOT emit a guessed composite name like '<entityA>_<entityB>' as missing — "
    ...
    "particular, an association table that holds BOTH a parent-record key AND a "
    "related-entity key (plus a row discriminator) lets you express 'the same "
    "related entity appearing in two roles on one parent' by a self-join / GROUP "
    "BY … HAVING on its own rows — no separate role/membership table is required. "
    "If such a table is present, the relationship IS satisfied → sufficient=true; "
    "reserve sufficient=false for when no slice table carries the needed keys at "
    "all.\n"
    ...
    "source reached over a present join path. Questions are often deliberately "
    "answerable through enrichment joins rather than a single conveniently-named "
    "column, so reaching for a literal column and degrading is the dominant false "
    "negative. A value is DERIVABLE/SUBSTITUTABLE when:\n"
    "  - A role/category the question asks for is carried by a different but "
    "semantically-equivalent column on a JOINED row (a type/category/status column "
    "on a related table, reached over a present join, supplies it) — do NOT "
    "require a literally-named role column when such an equivalent column is "
    "present on a related table reachable in the slice.\n"
    ...
    "it is empty or column-less. When an equivalent measure column on a related, "
    "joinable table IS present, the obviously-named table being empty or lacking "
    "the measure column is NOT a gap — use the equivalent source.\n"
    ...
    "keys (one table carrying both a parent-record key and a related-entity key, "
    "plus a row discriminator) — do NOT require a literally-named owner/role "
    "column or a dedicated role/membership table.\n"
