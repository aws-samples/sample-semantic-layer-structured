...
export class AthenaStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: AthenaStackProps) {
    ...
    NagSuppressions.addResourceSuppressions(this.spillBucket, [
      {
        id: 'AwsSolutions-S1',
        reason: 'Spill bucket stores temporary intermediate data from Athena DynamoDB connector queries; access logging is not required for ephemeral operational data',
      },
    ]);
