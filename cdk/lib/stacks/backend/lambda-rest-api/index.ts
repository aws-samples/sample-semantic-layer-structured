...
export class LambdaRestApiStack extends CommonStack {
  ...
  constructor(scope: Construct, id: string, props: LambdaRestApiStackProps) {
    ...
    // 2a. Dedicated DynamoDB table for async query tracking
    // Separate from ontology metadata — tracks status, S3 result key, TTL.
    // Full query results are stored in S3 (artifacts bucket); this table only holds metadata.
    const queryResultsTable = new dynamodb.Table(this, 'QueryResultsTable', {
      tableName: `${projectName}-query-results`,
    ...
    this.restApiFunction = new lambda.DockerImageFunction(this, 'RestApiFunction', {
      ...
      environment: {
        ...
        // Dedicated table for async query tracking (status + S3 result key)
        QUERY_RESULTS_TABLE: queryResultsTable.tableName,
        // AgentCore Runtime ARNs
    ...
    // Grant Lambda read/write on the query results tracking table
    queryResultsTable.grantReadWriteData(lambdaRole);
    ...
    // Catch-all route for /api/* (production via CloudFront)
    ...
    // Direct routes for local development (without /api prefix)
    // These allow the frontend to call the API Gateway directly during development
    const directRoutes = [
      '/ontology/{proxy+}',
      '/datasource/{proxy+}',
      '/query/{proxy+}',
