...
export interface FrontendStackProps extends StackProps {
  ...
  readonly enableSemanticRag?: boolean;
  /** Base URL of the AgentCore chat Gateway; streamChat posts here (bearer JWT). */
  readonly chatGatewayUrl?: string;
  /** MCP OAuth proxy URL (what Claude Code / Cursor connect to); shown on Settings. */
  readonly mcpProxyUrl?: string;
  /** MCP Gateway URL (the AgentCore Gateway behind the proxy); shown on Settings. */
...
export class FrontendStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: FrontendStackProps) {
    ...
    const buildEnvironment = {
      ...
      REACT_APP_CHAT_GATEWAY_URL: props.chatGatewayUrl || '',
      REACT_APP_MCP_PROXY_URL: props.mcpProxyUrl || '',
      REACT_APP_MCP_GATEWAY_URL: props.mcpGatewayUrl || '',
      ...
      REACT_APP_ENABLE_SEMANTIC_RAG: String(props.enableSemanticRag === true),
    };
    ...
    const frontendBuildProject = new codebuild.Project(this, 'FrontendBuild', {
      ...
      environment: {
        ...
        environmentVariables: {
          ...
          ...Object.entries(buildEnvironment).reduce(
            (acc, [key, value]) => {
              acc[key] = { value };
              return acc;
            },
      ...
      buildSpec: codebuild.BuildSpec.fromObject({
        ...
        phases: {
          ...
          build: {
            commands: ['echo "Building React application..."', 'CI=false npm run build'],
          },
          post_build: {
            commands: [
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        id: 'AwsSolutions-CB4',
        reason:
          'Frontend CodeBuild project builds React artifacts which are temporary and stored in S3 with encryption enabled; KMS encryption not required for ephemeral build outputs',
      },
      {
        id: 'AwsSolutions-SF1',
        reason:
          'Step Function state machine is auto-generated by CDK custom resource Provider for build orchestration; logging not required for internal CDK-managed resources',
      },
      {
        id: 'AwsSolutions-SF2',
        reason:
          'Step Function state machine is auto-generated by CDK custom resource Provider for build orchestration; X-Ray tracing not required for internal CDK-managed resources',
      },
      {
        id: 'AwsSolutions-SMG4',
        reason:
          'CloudFront domain secret contains the distribution domain name (derived from CloudFormation) and does not require rotation; this is computed infrastructure metadata, not user credentials',
      },
    ]);
  }
}
