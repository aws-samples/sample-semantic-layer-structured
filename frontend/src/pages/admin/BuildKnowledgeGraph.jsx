...
export default function BuildKnowledgeGraph({ user }) {
  ...
  useEffect(() => {
    if (!id) {
      setError(
        "No ontology ID provided. Please complete previous steps first.",
      );
    }
    return () => {
  ...
  return (
    <SpaceBetween size="l">
      <Header
        ...
        actions={<CancelDraftButton ontologyId={id} />}
      >
        ...
        <SpaceBetween size="l">
          {buildStatus === "idle" && (
            <Box>
              <SpaceBetween size="m">
                <Box variant="p">
                  Ready to build the knowledge graph. The AgentCore ontology
                  generation agent will:
                </Box>
                <ul>
                  <li>
                    Download and analyze uploaded reference documents (if
                    provided)
                  </li>
                  <li>
                    Extract schema metadata from AWS Glue for each selected
                    table
                  </li>
                  <li>
                    Retrieve relevant ontology design patterns from Bedrock
                    Knowledge Base
                  </li>
                  <li>
                    Incorporate business terminology from reference documents
                  </li>
                  <li>
                    Generate OWL ontology in N-QUADS format with Virtual
                    Knowledge Graph mappings
                  </li>
                  <li>
                    Persist ontology directly to Amazon Neptune graph database
                  </li>
                  <li>Save Turtle format backup to S3 for reference</li>
                </ul>
                <Alert type="info">
                  This process typically takes several minutes depending on the
                  number of tables, uploaded documents, and complexity of
                  relationships. You'll see real-time progress as each table is
                  processed.
                </Alert>
                <Box>
            ...
            <Box>
              <SpaceBetween size="m">
                ...
                <Box variant="small" color="text-status-inactive">
                  Please wait while the knowledge graph is being built. The
                  agent is processing tables, generating ontology classes and
                  properties, and persisting to Neptune.
                </Box>
              </SpaceBetween>
            </Box>
          )}
          ...
          {buildStatus === "completed" && (
            <Box>
              <SpaceBetween size="m">
                <Alert
                  type="success"
                  header="Knowledge Graph Built Successfully!"
                >
                  The AgentCore ontology agent has generated the OWL ontology
                  with Virtual Knowledge Graph mappings and persisted it to
                  Amazon Neptune. Successfully processed {totalTables} table
                  {totalTables !== 1 ? "s" : ""}. You can now view the knowledge
                  graph and start querying your data using natural language.
                </Alert>
                <ProgressBar
                  value={100}
              ...
              <SpaceBetween size="m">
                <Alert type="error" header="Build Failed">
                  ...
                  {tablesProcessed > 0 && (
                    <>
                      {" "}
                      Processed {tablesProcessed} of {totalTables} tables before
                      failure.
                    </>
                  )}{" "}
